...
#####
setwd("C:/Personal_Cloud/OneDrive/Work/Phd_Cyril/GIS_LAYERS/Scandinavia")
dem <- raster("dem_scan200.tif")
plot(dem)
setwd("C:/Personal_Cloud/OneDrive/Work/Skandulv/NHBD2/nhbd_2/gis")
study.area <- readOGR(".", "study_area")
scan <- readOGR(".", "countries")
scan <- scan[scan$NAME_0 %in% c("Norway","Sweden"),]
scan <- gSimplify(scan,topologyPreserve = T,tol = 500)
dem_s <- mask(dem,scan)
plot(scan)
plot(dem_s)
...
Female <- spTransform(Female, CRS(proj4string(dem_s)))

mcp_100 <- spTransform(mcp_100, CRS(proj4string(dem_s)))
...
pdf("Female_dispersal_map1.pdf", width = 6, height =6)
par(mar=c(0,0,0,0))
plot(study.area,border="white")
plot(dem_s,add=T,legend=FALSE,
     col=gray.colors(500, start = 0.2, end = 1
                     , gamma = 2.2, alpha = NULL))


plot(mcp_100,col=adjustcolor(col,alpha.f = 0.5),add=T, border=col)

for(i in 1:nrow(female.arraows)){
  arrows(x1=female.arraows[i,1],
           y1=female.arraows[i,2], 
           x0=female.arraows[i,3],
           y0=female.arraows[i,4], col="white",angle = 20,length = 0.08,code=1, lwd = 1.5)
}
points(female.arraows[,2]~female.arraows[,1], 
...
Male <- spTransform(Male, CRS(proj4string(dem_s)))
...
setwd("C:/Personal_Cloud/OneDrive/Work/Skandulv/NHBD2/figures")
pdf("Male_dispersal_map1.pdf", width = 6, height =6)
par(mar=c(0,0,0,0))
plot(study.area,border="white")
plot(dem_s,add=T,legend=FALSE,
     col=gray.colors(500, start = 0.2, end = 1
...
for(i in 1:nrow(female.arraows)){
  arrows(x1=Male.arraows[i,1],
         y1=Male.arraows[i,2], 
         x0=Male.arraows[i,3],
         y0=Male.arraows[i,4], col="white",angle = 20,length = 0.08,code=1, lwd = 1.5)
}
points(Male.arraows[,2]~Male.arraows[,1], 
       pch=21, col="black", bg="red")
dev.off()
...
setwd("C:/Personal_Cloud/OneDrive/Work/Skandulv/NHBD2/figures")
pdf("FemaleMALE_dispersal_map1.pdf", width = 7, height =4)
...
for(i in 1:nrow(female.arraows)){
  arrows(x1=female.arraows[i,1],
         y1=female.arraows[i,2], 
         x0=female.arraows[i,3],
         y0=female.arraows[i,4], col="white",angle = 20,length = 0.08,code=1, lwd = 1.1)
}
points(female.arraows[,2]~female.arraows[,1], 
       pch=21, col="black", bg="red")

mtext("A)", side=3, line=-20,adj=0.01)
...
for(i in 1:nrow(female.arraows)){
  arrows(x1=Male.arraows[i,1],
         y1=Male.arraows[i,2], 
         x0=Male.arraows[i,3],
         y0=Male.arraows[i,4], col="white",angle = 20,length = 0.08,code=1, lwd = 1.1)
}
points(Male.arraows[,2]~Male.arraows[,1], 
       pch=21, col="black", bg="red")

mtext("B)", side=3, line=-20,adj=0.01)
